...
function getAllAdmin(){
	...
	$sql="select id,username,email from life_admin ";
	$rows=fetchAll($sql);
	return $rows;
}
function getAdminByPage($page,$pageSize=2){
	...
	$sql="select id,username,email from life_admin limit {$offset},{$pageSize}";
	$rows=fetchAll($sql);
	return $rows;
}
...
function editAdmin($id){
	$arr=$_POST;
	$arr['password']=md5($_POST['password']);
	if(update("life_admin", $arr,"id={$id}")){
 ...
 */
function delUser($id){
	$sql="select face from life_user where id=".$id;
	$row=fetchOne($sql);
	$face=$row['face'];
	if(file_exists("../uploads/".$face)){
		unlink("../uploads/".$face);
	}
	if(delete("life_user","id={$id}")){
